...
System 2
Differs by:
    Uses porter stemmer instead of wordnet lemmatizer
    Creates word based tokens instead of sentence tokens
    Used Gensim stopwords corpus instead of NLTK stopwords#
    Doesnt convert text to lower
    Only indexes tokens
"""
import csv
import string
...
def tokens_nostopwords(s):
    ...
    nostopwords = [word for word in tokens if word not in punctuation]
    return nostopwords


def stemming(s):
    """
    Purpose: Applies the porter stemmer to an list of sentence tokens
    :param s: Raw text from a file
    :return stemmed array: List of sentence tokens with stemmed words and raw text
    """
    stemmed = []
    s_tokens = tokens_nostopwords(s)
...
def search(field, parameter):
    ...
    result = es.search(index='covid_data_system2', body=query, size=1000)
    # pprint(result)
    return result
...
def format_search(results, field):
    ...
    for doc in result_data[:10]:
        print("\n(ID: %s)" % (doc['_source']['cord_uid']) + "\nSearch Result: %s" % (doc['_source'][field]) + "\nTF-IDF Score: %s" % (
            doc['_score']))
        top10.append(doc['_source']['cord_uid'])
    for i in top10:
        print(i[0], sep='\n')
...
with open('../metadata.csv', encoding='utf-8') as f:
    index_name = 'covid_data_system2'
    doctype = 'covid_open_research_data_sys2'
    reader = csv.reader(f)
