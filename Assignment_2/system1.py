"""
Reimplemented TF-IDF as a custom function
"""
import csv
import string
import sys
...
def search(field, parameter):
    ...
    result = es.search(index='covid_data_system1', body=query, size=1000)
    # pprint(result)
    return result
                ...
                "settings": {
                    ...
                    "similarity": {
                        "tfidf": {
                            "type": "scripted",
                            "script": {
                                "source": "double tf = Math.sqrt(doc.freq); double idf = Math.log(("
                                          "field.docCount+1.0)/(term.docFreq+1.0)) + 1.0; double norm = 1/Math.sqrt("
                                          "doc.length); return query.boost * tf * idf * norm; "
                            }
                        }
                    }
                },
